...
function copyDir(src, dest) {
  ...
  for (const entry of entries) {
    ...
    // Skip .git (both directory in full clones and file in submodules)
 ...
 * @param {string} [expectedName] - Optional expected skill name (for root-level skills where dir name != skill name)
 */
function validateSkill(skillDir, expectedName) {
  ...
  const errors = [];
  const warnings = [];
  const dirName = expectedName || path.basename(skillDir);
 ...
 * @param {string} skillDir - Path to the skill directory
 * @param {string} outputBase - Base output directory for plugins
 * @param {object} validation - Validation result from validateSkill
 * @param {string} sourceAuthor - Author from config source (fallback)
 * @param {string} [overrideName] - Optional override for the plugin/skill name (used for root-level skills)
 */
function convertToPlugin(skillDir, outputBase, validation, sourceAuthor, overrideName) {
  const skillName = overrideName || path.basename(skillDir);
  ...
  for (const source of config.sources) {
    ...
    if (source.skills && Array.isArray(source.skills)) {
      for (const skillGroup of source.skills) {
        // ── Root-level skill (SKILL.md at repo root) ────────────────────
        // If no "directory" is specified, the skill is at the repo root
        if (!skillGroup.directory) {
          const repoRootDir = repoDir;

          // Check that SKILL.md exists at repo root
          if (!fs.existsSync(path.join(repoRootDir, 'SKILL.md'))) {
            log(`No SKILL.md found at root of ${source.author}/${source.repo}`, 'error');
            totalErrors++;
            continue;
          }
          ...
          for (const skillName of expectedSkills) {
            // Validate using the expected name so directory-name check makes sense
            const validation = validateSkill(repoRootDir, skillName);
            if (!validation.valid) {
            ...
            convertToPlugin(repoRootDir, tempPluginDir, validation, source.author, skillName);
            ...
            removeDir(pluginDir);
            convertToPlugin(repoRootDir, outputDir, validation, source.author, skillName);
            log(`Skill "${skillName}" — converted and saved ✅`, 'success');
            totalConverted++;
            changes.push(skillName);
          }
          continue; // Skip directory-based logic for root skills
        }
    ...
    // ── Pre-built plugins (no conversion needed) ──────────────────────
    if (source.plugins && Array.isArray(source.plugins)) {
      for (const pluginEntry of source.plugins) {
        const pluginNames = pluginEntry.names || [];

        if (pluginNames.length === 0) {
          log(`Plugin entry in ${source.author}/${source.repo} has empty names array — skipping`, 'warn');
          continue;
        }

        // Resolve source directory once (same for all names in this entry)
        const pluginSourceDir = pluginEntry.directory
          ? path.join(repoDir, pluginEntry.directory)
          : repoDir;
        ...
        for (const pluginName of pluginNames) {
          ...
          if (!fs.existsSync(pluginJsonPath)) {
            log(`Plugin "${pluginName}" missing .claude-plugin/plugin.json in ${source.author}/${source.repo}`, 'error');
            totalErrors++;
            continue;
          }

          // Read plugin name from manifest for display
          let manifestName = pluginName;
          try {
            const manifest = JSON.parse(fs.readFileSync(pluginJsonPath, 'utf-8'));
            manifestName = manifest.name || manifest.id || pluginName;
          } catch { /* use config name as fallback */ }

          const pluginOutputDir = path.join(outputDir, pluginName);

          // Hash-based change detection
          const currentHash = getPluginHash(pluginOutputDir);
          const newHash = getPluginHash(pluginSourceDir);

          if (!FLAG_FORCE && currentHash === newHash && currentHash !== '') {
            log(`Plugin "${manifestName}" — unchanged, skipped`);
            totalSkipped++;
            continue;
          }

          if (FLAG_DRY_RUN) {
            log(`Plugin "${manifestName}" — would copy (dry run)`);
            totalConverted++;
            changes.push(pluginName);
            continue;
          }

          // Copy the plugin directory as-is
          removeDir(pluginOutputDir);
          copyDir(pluginSourceDir, pluginOutputDir);
          log(`Plugin "${manifestName}" — copied and saved ✅`, 'success');
          totalConverted++;
          changes.push(pluginName);
        }
      }
